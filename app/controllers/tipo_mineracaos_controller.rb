class TipoMineracaosController < ApplicationController
  ...
  def index
    @tipo_mineracaos = TipoMineracao.all.paginate(:page => params[:page], :per_page => 02)
  end
  ...
  def create
    ...
    respond_to do |format|
      if @tipo_mineracao.save
        format.html { redirect_to tipo_mineracao_url(@tipo_mineracao), notice: "Tipo mineração cadastrado com sucesso." }
  ...
  def update
    respond_to do |format|
      if @tipo_mineracao.update(tipo_mineracao_params)
        format.html { redirect_to tipo_mineracao_url(@tipo_mineracao), notice: "Tipo mineração atualizado com sucesso." }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to tipo_mineracaos_url, notice: "Tipo mineração deletado com sucesso." }
